...
$.post("control", { "func":"getState","lastgame":0,"lastmessage":0}, updateState,"json"); 

/*
$('#seat1 div.seattext1 ').html("player1111");
...
{
	$('#gameField img')
                        .eq(Math.floor(Math.random()*52))
                        .clone()
                        //.attr("height","90px")
                        .appendTo('#seat'+i+' div.seatcards')
                        .css('z-index', ++tPos)
                        .addClass('player1')
                        .fadeIn(0)
                        .css("top", "0px")
                        .css("left", (j*spacing)+"px");
}
}*/
/*
$('#gameField img')
...
function updateState(data)
{
	...
	if(lastgame == 0)
	{
		lastgame = data.gameNum;
	}

	for(var i = 0; i < data.messages.length; i++)
	{
		if(data.messages[i].messageNum != lastMessage + 1)
		{
			alert("lastMessage error -1");
			return;
		}
		lastMessage++;

		if(data.messages[i].command == "gameOver")
		{
			process_gameOver(data.messages[i]);
		}
		else if(data.messages[i].command == "updateChat")
		{
			process_updateChat(data.messages[i]);
		}
		else if(data.messages[i].command == "updateTable")
                {
                        process_updateTable(data.messages[i]);
                }
		else if(data.messages[i].command == "yourMove")
                {
                        process_yourMove(data.messages[i]);
                }
		else
		{
			alert("Invalid message command received: " + data.messages[i].command);
			return;
		}
